...
$(document).ready(function() { 
	...
	$('#column-right').bind('scrollstop', function() {
		$('#column-right a').each(function(index, element) {
			...
			if ((offset.top > 0) && (offset.top < height) && $(element).find('img').attr('src') == '<?=$no_image;?>') {
				$.ajax({
					url: 'index.php?route=common/filemanager/image&token=<?=$token;?>&image=' + encodeURIComponent('data/' + $(element).find('input[name=\'image\']').attr('value')),
					dataType: 'html',
					...
					success: function(json) {
						...
						if (json) {
							for (i = 0; i < json.length; i++) {
								html += '<a><img src="<?=$no_image;?>" alt="" title="" /><br />' + ((json[i]['filename'].length > 15) ? (json[i]['filename'].substr(0, 15) + '..') : json[i]['filename']) + '<br />' + json[i]['size'] + '<input type="hidden" name="image" value="' + json[i]['file'] + '" /></a>';
							}
						}
   ...
   $('#delete').bind('click', function() {
	...
	new AjaxUpload('#upload', {
		action: 'index.php?route=common/filemanager/upload&token=<?=$token;?>',
