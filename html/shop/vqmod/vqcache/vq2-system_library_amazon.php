...
class Amazon { 
     ...
     */
     public function getLinkedSkus($productId, $var='') {
        return $this->db->query("SELECT `amazon_sku`
            FROM `" . prefix . "amazon_product_link`
    ...
    public function deleteProduct($product_id){
        $this->db->query("DELETE FROM `" . prefix . "amazon_product_link` WHERE `product_id` = '" . $this->db->escape($product_id) . "'");
    }
    ...
    public function getOrder($orderId) {
        $qry = $this->db->query("SELECT * FROM `" . prefix . "amazon_order` WHERE `order_id` = '".(int)$orderId."' LIMIT 1");
