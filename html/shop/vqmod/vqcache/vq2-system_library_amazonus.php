...
class Amazonus {
    ...
    public function putStockUpdateBulk($productIdArray, $endInactive = false){
        ...
        foreach($productIdArray as $productId) {
            ...
            foreach($amazonusRows as $amazonusRow) {
                $productRow = $this->db->query("SELECT quantity, status FROM `" . prefix . "product`
     ...
     */
     public function getLinkedSkus($productId, $var='') {
        return $this->db->query("SELECT `amazonus_sku`
            FROM `" . prefix . "amazonus_product_link`
        ...
        foreach ($order_product_query->rows as $order_product) {
            ...
            if (isset($product_query->row['has_option']) && ($product_query->row['has_option'] == 1)) {
                ...
                if ($pOption_query->num_rows != 0) {
                    ...
                    $qtyLeftRow = $this->db->query("SELECT `stock` FROM `" . prefix . "product_option_relation` WHERE `product_id` = '" . (int)$order_product['product_id'] . "' AND `var` = '" . $this->db->escape($var) . "'")->row;
                    if(empty($qtyLeftRow)) {
                        $qtyLeftRow['stock'] = 0;
                    }
        ...
        if(file_exists(DIR_SYSTEM."ebay_addon/".$addon.".php"))
        {
            if($addon == "openstock") {
                $isInstalled = $this->db->query("
                    SELECT COUNT(*) as count FROM `" . prefix . "extension`
    ...
    public function deleteProduct($product_id){
        $this->db->query("DELETE FROM `" . prefix . "amazonus_product_link` WHERE `product_id` = '" . $this->db->escape($product_id) . "'");
    }
    ...
    public function getOrder($orderId) {
        $qry = $this->db->query("SELECT * FROM `" . prefix . "amazonus_order` WHERE `order_id` = '".(int)$orderId."' LIMIT 1");
