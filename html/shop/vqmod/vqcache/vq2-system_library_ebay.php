...
final class Ebay
{
    ...
    public function getSetting($key){
        $qry = $this->db->query("SELECT * FROM `" . prefix . "ebay_setting_option` WHERE `key` = '".$key."' LIMIT 1");
    ...
    public function getEbayItemId($product_id){
        ...
        */
        ...
        $qry = $this->db->query("SELECT `ebay_item_id` FROM `" . prefix . "ebay_listing` WHERE `product_id` = '".$product_id."' AND `status` = '1' LIMIT 1");
    ...
    public function getEndedEbayItemId($product_id){
        ...
        $qry = $this->db->query("SELECT `ebay_item_id` FROM `" . prefix . "ebay_listing` WHERE `product_id` = '".$product_id."' AND `status` = '0' ORDER BY `ebay_listing_id` DESC LIMIT 1");
    ...
    public function removeItemId($id){
        ...
        $this->db->query("UPDATE `" . prefix . "ebay_listing` SET `status` = '0' WHERE `ebay_item_id` = '".$this->db->escape($id)."'");
        $this->db->query("DELETE FROM `" . prefix . "ebay_stock_reserve` WHERE `item_id` = '".$this->db->escape($id)."'");
    }
    ...
    public function deleteProduct($product_id){
        ...
        $this->db->query("DELETE FROM `" . prefix . "ebay_listing` WHERE `product_id` = '".(int)$product_id."'");
        $this->db->query("DELETE FROM `" . prefix . "ebay_stock_reserve` WHERE `product_id` = '".(int)$product_id."'");
    }
     ...
     */
        ...
        $qry = $this->db->query("SELECT `product_id`, `ebay_item_id` FROM `" . prefix . "ebay_listing` WHERE `status` = '1'");
    ...
    public function getEndedListingArray(){
        ...
        $qry = $this->db->query("SELECT e.*
            FROM
                ...
                FROM `" . prefix . "ebay_listing`
                WHERE `status` = 0
                GROUP BY `product_id`) `a`
            INNER JOIN `" . prefix . "ebay_listing` `e`
     ...
     */
        ...
        FROM `" . prefix . "ebay_listing` `el` 
        LEFT JOIN `" . prefix . "product` `p` ON (`el`.`product_id` = `p`.`product_id`) 
     ...
     */
        ...
        if(!empty($product_id)){
            if($sku == null){
                $query = $this->db->query("SELECT * FROM `" . prefix . "product` WHERE `product_id` = '".(int)$product_id."' LIMIT 1");
                ...
                $query = $this->db->query("SELECT * FROM `" . prefix . "product_option_relation` WHERE `product_id` = '".(int)$product_id."' AND `var` = '".$sku."' LIMIT 1");
    ...
    public function isEbayOrder($id){
        ...
        $qry = $this->db->query("SELECT `comment` FROM `" . prefix . "order_history` WHERE `comment` LIKE '[eBay Import:%]' AND `order_id` = '".$id."' LIMIT 1");
    ...
    public function orderNew($order_id){
        ...
        if(!$this->isEbayOrder($order_id)){
            if($this->addonLoad('openstock') == true){
                ...
                $order_product_query = $this->db->query("SELECT * FROM `" . prefix . "order_product` WHERE `order_id` = '" . (int)$order_id . "'");
    ...
    private function osProducts($order_id){
        ...
        foreach ($order_product_query->rows as $order_product) {
            $product_query = $this->db->query("SELECT * FROM " . prefix . "product WHERE `product_id` = '" .(int)$order_product['product_id']."' LIMIT 1");

            if (isset($product_query->row['has_option']) && ($product_query->row['has_option'] == 1)) {
                $pOption_query = $this->db->query("
                        SELECT `" . prefix . "order_option`.`product_option_value_id`
                        FROM `" . prefix . "order_option`, `" . prefix . "product_option`, `" . prefix . "option`
                        WHERE `" . prefix . "order_option`.`order_product_id` = '" . (int) $order_product['order_product_id'] . "'
                        AND `" . prefix . "order_option`.`order_id` = '" . (int) $order_id . "'
                        AND `" . prefix . "order_option`.`product_option_id` = `" . prefix . "product_option`.`product_option_id`
                        AND `" . prefix . "product_option`.`option_id` = `" . prefix . "option`.`option_id`
                        AND ((`" . prefix . "option`.`type` = 'radio') OR (`" . prefix . "option`.`type` = 'select'))
                        ORDER BY `" . prefix . "order_option`.`order_option_id`
                        ASC");
    ...
    public function getProductStockLevel($productId, $sku = ''){
        ...
        if($sku == ''){
            $qry = $this->db->query("SELECT `quantity`, `status` FROM `" . prefix . "product` WHERE `product_id` = '".$productId."' LIMIT 1");

            return array('quantity' => (int)$qry->row['quantity'], 'status' => ($qry->row['status']));
        }else{
            $qry = $this->db->query("SELECT `stock`, `active` FROM `" . prefix . "product_option_relation` WHERE `product_id` = '".$productId."' AND `var` = '".$this->db->escape($sku)."' LIMIT 1");
        ...
        if($ebay_id != false){
            ...
            if(!empty($item_txn_array)){
                ...
                if($status_id == $this->config->get('EBAY_DEF_SHIPPED_ID')){
                    ...
                    $qry = $this->db->query("UPDATE `" . prefix . "ebay_order` SET `carrier_id` = '".$this->db->escape((isset($data['carrier_id']) ? $data['carrier_id'] : ''))."', `tracking_no` = '".$this->db->escape((isset($data['tracking_no']) ? $data['tracking_no'] : ''))."' WHERE `order_id` = '".(int)$order_id."' LIMIT 1");
                }
    ...
    public function getProductId($ebay_item){
        ...
        $qry = $this->db->query("SELECT `product_id` FROM `" . prefix . "ebay_listing` WHERE `ebay_item_id` = '".$ebay_item."' LIMIT 1");
    ...
    public function getProductIdFromKey($key){
        $qry = $this->db->query("SELECT `product_id` FROM `" . prefix . "ebay_listing_pending` WHERE `key` = '".$key."' LIMIT 1");
    ...
    public function getAllocatedStock($product_id){
        $qry = $this->db->query("SELECT SUM(`qty`) AS `total` FROM `" . prefix . "ebay_transaction` WHERE `product_id` = '".$product_id."' AND `order_id` = '0' LIMIT 1");
        return (int)$qry->row['total'];
    }
    
    public function getImages(){
        $this->log('getImages() - Getting product images.');
        $qry = $this->db->query("SELECT * FROM `" . prefix . "ebay_image_import`");
        ...
        if($qry->num_rows){
            foreach ($qry->rows as $img) {
                ...
                if($img['imgcount'] == 0){
                    ...
                    $this->db->query("UPDATE `" . prefix . "product` SET `image` = 'data/".$img['name']."' WHERE `product_id` = '".$img['product_id']."' LIMIT 1");
                }else{
                    //add to gallery table
                    $this->db->query("
                        INSERT INTO `" . prefix . "product_image` SET
                ...
                $this->db->query("DELETE FROM `" . prefix . "ebay_image_import` WHERE `id` = '".$img['id']."' LIMIT 1");
            }
        }
    }
    ...
    public function createLink($product_id, $item_id, $variant){
        ...
        $this->db->query("
            INSERT INTO `" . prefix . "ebay_listing`
            SET
    ...
    public function insertReserve($data, $item_id, $variant){
        if($variant == 1){
            foreach($data['opt'] as $variation){
                $query = $this->db->query("SELECT * FROM `" . prefix . "product_option_relation` WHERE `product_id` = '".(int)$data['product_id']."' AND `var` = '".$variation['sku']."' LIMIT 1");

                if($query->row['stock'] != $variation['qty']){

                    $this->db->query("
                    INSERT INTO `" . prefix . "ebay_stock_reserve`
                    SET
            ...
            $query = $this->db->query("SELECT * FROM `" . prefix . "product` WHERE `product_id` = '".(int)$data['product_id']."' LIMIT 1");
    ...
    public function getReserve($product_id, $item_id, $sku = ''){
        $this->log('getReserve()');
        $query = $this->db->query("SELECT * FROM `" . prefix . "ebay_stock_reserve` WHERE `product_id` = '".(int)$product_id."' AND `variant_id` = '".$this->db->escape($sku)."' AND `item_id` = '".$this->db->escape($item_id)."'  LIMIT 1");
    ...
    public function updateReserve($product_id, $item_id, $reserve, $sku = '', $variant = 0){
        ...
        if($reserve == 0){
            ...
            if($this->getReserve($product_id, $item_id, $sku) != false){
                $this->db->query("UPDATE `" . prefix . "ebay_stock_reserve` SET `reserve` = '".(int)$reserve."' WHERE `product_id` = '".(int)$product_id."' AND `variant_id` = '".$this->db->escape($sku)."' AND `item_id` = '".$this->db->escape($item_id)."'  LIMIT 1");
            }else{
                if($variant == 0){
    ...
    public function deleteReserve($product_id, $item_id, $sku = ''){
        ...
        $this->db->query("DELETE FROM `" . prefix . "ebay_stock_reserve` WHERE `product_id` = '".(int)$product_id."' AND `variant_id` = '".$this->db->escape($sku)."' AND `item_id` = '".$this->db->escape($item_id)."'  LIMIT 1");
    }

    public function getCarriers(){
        $qry = $this->db->query("SELECT * FROM " . prefix . "ebay_shipping");
    ...
    public function getOrder($order_id){
        if($this->openbay->testDbTable(prefix . "ebay_order") == true){
            $qry = $this->db->query("SELECT * FROM `" . prefix . "ebay_order` WHERE `order_id` = '".(int)$order_id."' LIMIT 1");
    ...
    public function loadCategories(){
        ...
        if($this->lasterror != true){
            $this->db->query("TRUNCATE TABLE `" . prefix . "ebay_category`");
            ...
            if(!empty($cat_array)){
                foreach($cat_array as $cat){
                    ...
                    $cat_qry = $this->db->query("
                        INSERT INTO `" . prefix . "ebay_category`
    ...
    public function loadSettings() {
        ...
        if ($this->lasterror === false){
            if (isset($response['urls']['ViewItemURL'])) {
                $this->db->query("DELETE FROM `" . prefix . "setting` WHERE  `key` = 'openbaypro_ebay_itm_link' LIMIT 1");
                $this->db->query("INSERT INTO `" . prefix . "setting` SET `value` = '" . $this->db->escape((string) $response['urls']['ViewItemURL']) . "', `key` = 'openbaypro_ebay_itm_link', `group` = 'openbay'");
            ...
            if(isset($response['payment_options'])){
                $this->db->query("TRUNCATE TABLE `" . prefix . "ebay_payment_method`");
                $this->log('Emptied ebay_payment_method table');

                foreach ($response['payment_options'] as $child) {
                    $this->db->query("
                        INSERT INTO `" . prefix . "ebay_payment_method`
                        SET
            ...
            if (isset($response['shipping_service'])) {
                $this->db->query("TRUNCATE TABLE `" . prefix . "ebay_shipping`");
                ...
                foreach ($response['shipping_service'] as $service) {
                    ...
                    $this->db->query("
                        INSERT INTO `" . prefix . "ebay_shipping`
                        SET
            ...
            if (isset($response['shipping_location'])) {
                $this->db->query("TRUNCATE TABLE `" . prefix . "ebay_shipping_location`");
                $this->log('Emptied ebay_shipping_location table');
                foreach ($response['shipping_location'] as $service) {
                    $this->db->query("
                        INSERT INTO `" . prefix . "ebay_shipping_location`
                        SET
                            `description`         = '" . $this->db->escape((string) $service['Description']) . "',
            ...
            if (isset($response['exclude_shipping_location'])) {
                $this->db->query("TRUNCATE TABLE `" . prefix . "ebay_shipping_location_exclude`");
                $this->log('Emptied ebay_shipping_location_exclude table');
                foreach ($response['exclude_shipping_location'] as $service) {
                    $this->db->query("
                        INSERT INTO `" . prefix . "ebay_shipping_location_exclude`
                        SET
                            `description`   = '" . $this->db->escape((string) $service['Description']) . "',
            ...
            if (isset($response['dispatch_time_max'])) {
                $qry = $this->db->query("SELECT * FROM `" . prefix . "ebay_setting_option` WHERE `key` = 'dispatch_time_max' LIMIT 1");

                if ($qry->num_rows > 0) {
                    $this->db->query("
                        UPDATE `" . prefix . "ebay_setting_option`
                        SET
                    ...
                    //insert
                    $this->db->query("
                        INSERT INTO `" . prefix . "ebay_setting_option`
                        SET
    ...
    public function loadSellerStore(){
        ...
        if($this->lasterror != true){
            if($store['store'] == true){
                $this->db->query("DROP TABLE IF EXISTS `" . prefix . "ebay_store_category`;");
                $this->db->query("
                            CREATE TABLE IF NOT EXISTS `" . prefix . "ebay_store_category` (
                ...
                if(!empty($store['settings']['categories'])){
                    foreach($store['settings']['categories'] as $cat1){
                        $this->db->query("INSERT INTO `" . prefix . "ebay_store_category` SET `CategoryID` = '".$cat1['id']."', `CategoryName` = '".$this->db->escape($cat1['name'])."'");
                        $id1 = $this->db->getLastId();

                        if(!empty($cat1['children'])){
                            foreach($cat1['children'] as $cat2){
                                $this->db->query("INSERT INTO `" . prefix . "ebay_store_category` SET `CategoryID` = '".$cat2['id']."', `CategoryName` = '".$this->db->escape($cat2['name'])."', `parent_id` = '".$id1."'");
                                $id2 = $this->db->getLastId();

                                if(!empty($cat2['children'])){
                                    foreach($cat2['children'] as $cat3){
                                        $this->db->query("INSERT INTO `" . prefix . "ebay_store_category` SET `CategoryID` = '".$cat3['id']."', `CategoryName` = '".$this->db->escape($cat3['name'])."', `parent_id` = '".$id2."'");
                                        $id3 = $this->db->getLastId();

                                        if(!empty($cat3['children'])){
                                            foreach($cat3['children'] as $cat4){
                                                $this->db->query("INSERT INTO `" . prefix . "ebay_store_category` SET `CategoryID` = '".$cat4['id']."', `CategoryName` = '".$this->db->escape($cat4['name'])."', `parent_id` = '".$id3."'");
    ...
    public function editSetting($group, $data, $store_id = 0) {
            $this->db->query("DELETE FROM " . prefix . "setting WHERE store_id = '" . (int)$store_id . "' AND `group` = '" . $this->db->escape($group) . "'");

            foreach ($data as $key => $value) {
                    if (!is_array($value)) {
                            $this->db->query("INSERT INTO " . prefix . "setting SET store_id = '" . (int)$store_id . "', `group` = '" . $this->db->escape($group) . "', `key` = '" . $this->db->escape($key) . "', `value` = '" . $this->db->escape($value) . "'");
                    } else {
                            $this->db->query("INSERT INTO " . prefix . "setting SET store_id = '" . (int)$store_id . "', `group` = '" . $this->db->escape($group) . "', `key` = '" . $this->db->escape($key) . "', `value` = '" . $this->db->escape(serialize($value)) . "', serialized = '1'");
                    }
            }
    }

    public function getShippingServiceInfo($service_code){
        $qry = $this->db->query("SELECT * FROM `" . prefix . "ebay_shipping` WHERE `ShippingService` = '".$this->db->escape($service_code)."' LIMIT 1");
