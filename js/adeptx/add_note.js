function add_note(ni)
{
 var $note;

 $('body').append('<div class=note id=n'+ni+'>\
  <div class=note_name>Заметка</div>\
  <div class=note_main>\
   <textarea id=note'+ni+' class=note_text onkeyup="change(this);"></textarea>\
  </div>\
  <div class=resize></div>\
 </div>');
 var $note = $(".note:last");

 $('.note_name:last').mousedown(function(){move($(this).parent());});
 $('.note_name:last').dblclick(function(){slide($(this).next(),345);});
 conf(0,ni);

 var exist=0;var doit=0;

 $note.css({'left':note[ni].left,'top':note[ni].top,'width':note[ni].width,'height':note[ni].height,'z-index':maxZ++});

 $note.mousedown(function() /* определение смен z-index-ов - меняется на меньший у всего класса, меняется на больший у одного. */
 {
  if ($(this).css('z-index') != (maxZ-1)) $(this).css('z-index',maxZ++);
/*
  do
  {
   var doit=0;
   for(var i=1; i<maxZ; i++)
   {
    ...
    {
     if ($(this).css('z-index') == i) {exist=1;doit=1;}
    });
    if (!exist) $('.note').each(function(n)
    {//alert(i);
     if ($(this).css('z-index') > i) $(this).css('z-index',$(this).css('z-index')-1);
    });
   }
  }
  while(doit);*/
